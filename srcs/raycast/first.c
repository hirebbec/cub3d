...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/06/05 15:31:33 by hirebbec          #+#    #+#             */
/*   Updated: 2022/06/29 16:53:51 by hirebbec         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	hit_c_first(double x, double y, t_game *game, double angle)
{
	int	x_;
	int	y_;
...
int	hit_l_first(double x, double y, t_game *game, double angle)
{
	int	x_;
	int	y_;
...
{
	first_line(game, &ray, luch);
	while (!hit_l_first(ray.x, ray.y, game, game->gamma - (ray.angle * 0.0174)))
	{
		game->x_prev = ray.x;
		game->y_prev = ray.y;
...
{
	first_colmn(game, &ray, luch);
	while (!hit_c_first(ray.x, ray.y, game, game->gamma + (ray.angle * 0.0174)))
	{
		game->x_prev = ray.x;
		game->y_prev = ray.y;
		if (game->gamma + (ray.angle * 0.0174) > 1.570796)
		{
			...
			else_first_colmn(game, &ray, luch);
	}
	luch->x_hit = ray.x;
	luch->y_hit = ray.y;
	return (pifogor(fabs(game->player.x - ray.x), fabs(game->player.y - ray.y), \
